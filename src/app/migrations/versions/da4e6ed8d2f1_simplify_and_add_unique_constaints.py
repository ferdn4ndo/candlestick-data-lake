...
def upgrade():
    ...
    op.add_column('currency_pair', sa.Column('currency_base_id', sa.Integer(), nullable=True))
    op.add_column('currency_pair', sa.Column('currency_quote_id', sa.Integer(), nullable=True))
    ...
    op.create_foreign_key(None, 'currency_pair', 'currency', ['currency_base_id'], ['id'])
    op.create_foreign_key(None, 'currency_pair', 'currency', ['currency_quote_id'], ['id'])
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('currency_pair', sa.Column('currency_a_id', mysql.INTEGER(display_width=11), autoincrement=False, nullable=True))
    op.add_column('currency_pair', sa.Column('currency_b_id', mysql.INTEGER(display_width=11), autoincrement=False, nullable=True))
    op.drop_constraint(None, 'currency_pair', type_='foreignkey')
    op.drop_constraint(None, 'currency_pair', type_='foreignkey')
    op.create_foreign_key('currency_pair_ibfk_2', 'currency_pair', 'currency', ['currency_b_id'], ['id'])
    op.create_foreign_key('currency_pair_ibfk_1', 'currency_pair', 'currency', ['currency_a_id'], ['id'])
    ...
    op.add_column('currency', sa.Column('precision', mysql.INTEGER(display_width=11), autoincrement=False, nullable=True))
    op.alter_column('currency', 'name',
               existing_type=mysql.VARCHAR(length=20),
               nullable=False)
